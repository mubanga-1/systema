...
export function AuthTabs({ loginLabel, signupLabel }: AuthTabsProps) {
  const pathname = usePathname();
  const loginActive = pathname === '/login';
  const signupActive = pathname === '/register';

  return (
    <div className="flex items-center gap-2">
      <Link href="/login" className={getTabClassName(loginActive)}>
        {loginLabel}
      </Link>
      <Link href="/register" className={getTabClassName(signupActive)}>
        {signupLabel}
      </Link>
    </div>
  );
